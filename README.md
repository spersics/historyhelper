...
- Два режима хранения истории: "Тип хранения по умолчанию" и "Оптимизированный тип хранения".
- Просмотр сгенерированного SQL и **применение** его прямо из DBeaver.
- Фильтрация системных колонок (При генерации возможно выбрать только пользовательские колонки).
- Удобное диалоговое окно + иконка команды в контекстном меню таблицы.
- Возможность создания опциональных столбцов: столбец юзера (который выполнил действие), столбец даты действия, столбец действия (INSERT/UPDATE/DELETE).
- Оптимизированное хранение. Есть опция выбора хранения полной истории, либо хранения только предпоследнего значения записи.
- Поддержка СУБД Postgres и MySql (Плагин сам определяет диалект вашей СУБД)
...
2. Поместите файл в папку 'plugins' или 'dropins' установленного DBeaver:
   **При установке новой версии - удалите старый файл**
...
**Если при переустановке новой версии вы столкнулись с проблемой**, что не можете открыть плагин:
Зайдите по пути 'C:\ваш_путь_до_dbeaver\DBeaver\configuration\org.eclipse.equinox.simpleconfigurator' и в файле
bundles.info вручную измените версию плагина на ту, которую вы устанавливаете.
...
3. В диалоговом окне:
   ...
   - при необходимости переключите режим хранения
